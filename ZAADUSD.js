...
import { View, SafeAreaView, Image, Text, StyleSheet, TouchableOpacity, TextInput, Alert, ScrollView, Linking } from 'react-native';
import COLORS from './colors';

const ZAADUSDSCREEN = ({ route }) => {
  ...
  const handlePress = async () => {
    ...
    const code = `*277*323401*${lastNumber}#`;
    const encodedCode = encodeURIComponent(code);
    const url = `tel:${encodedCode}`;
    ...
    <ScrollView>
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.detailsContainer}>
          <Text style={styles.title}>US Dollar ayaad heli doontaa</Text>
